...
def run_preprocessing(df):
    ...
    # Normalisation
    apply_scaling = st.checkbox("Normaliser les données")
    # Scaling
    scaler = None
    if apply_scaling:
    ...
    remove_outliers = st.checkbox("Supprimer les outliers")
    # Outlier Removal
    if remove_outliers:
        Q1 = X_train.quantile(0.25)
        Q3 = X_train.quantile(0.75)
        IQR = Q3 - Q1
        filter = ~((X_train < (Q1 - 1.5 * IQR)) | (X_train > (Q3 + 1.5 * IQR))).any(axis=1)
